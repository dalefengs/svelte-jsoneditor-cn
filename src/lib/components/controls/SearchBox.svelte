      ...
      <div class="jse-search-contents">
        ...
        {#if showReplace && !readOnly}
          <div class="jse-replace-section">
            ...
            <button
              type="button"
              title="替换当前出现的内容（Ctrl+Enter）"
              on:click={handleReplace}>替换</button
            >
            <button type="button" title="替换所有出现" on:click={handleReplaceAll}
              >全部替换</button
            >
          </div>
        {/if}
